...
class unFlowLoss(nn.modules.Module):
    ...
    def loss_photomatric(self, im1_scaled, im1_recons, occu_mask1):
        ...
        if self.args.w_l1 > 0:
            loss += [self.args.w_l1 * (im1_scaled - im1_recons).abs() * occu_mask1]

        if self.args.w_ssim > 0:
            loss += [self.args.w_ssim * SSIM(im1_recons * occu_mask1,
                                            im1_scaled * occu_mask1)]

        if self.args.w_ternary > 0:
            loss += [self.args.w_ternary * TernaryLoss(im1_recons * occu_mask1,
                                                      im1_scaled * occu_mask1)]

        return sum([l.mean() for l in loss]) / occu_mask1.mean()

    def loss_smooth(self, flow, im1_scaled):
        if 'smooth_2nd' in self.args and self.args.smooth_2nd:
            func_smooth = smooth_grad_2nd
        else:
            func_smooth = smooth_grad_1st
        loss = []
        loss += [func_smooth(flow, im1_scaled, self.args.alpha)]
